...
import { generateSceneBackground, listSceneLibrary, polishSceneText } from "../lib/api";
...
function normalizeSceneUrl(value) {
  ...
  if (String(value).startsWith("/textures/")) return value;
...
function sceneLibraryStatus(source, refreshed = false) {
  if (source === "unsplash") {
    return refreshed ? "已刷新在线场景图库。" : "已载入在线场景图库。";
  }
  if (source === "mixed") {
    return refreshed
      ? "已刷新场景图库，当前优先显示本地预设并补充在线图片。"
      : "当前优先显示本地预设场景，并补充在线场景图库。";
  }
  return "当前使用本地预设场景图库。";
}
...
export default function ScenePreviewPage() {
  ...
  const { speechSupported, listening, toggleSpeechInput } = useSpeechInput({
    ...
    startHint: "正在采集背景描述，点击结束。",
    doneHint: "语音已识别并填入背景描述。",
  });
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        setStatus(sceneLibraryStatus(data.source, false));
      } catch (error) {
        if (!mounted) return;
  ...
  async function refreshLibrary() {
    ...
    try {
      ...
      const first = next[0]?.full_url || next[0]?.thumb_url || "";
      setSelectedUrl(first);
      setStatus(sceneLibraryStatus(data.source, true));
    } catch (error) {
  ...
  async function handleGenerateBackground() {
    ...
    try {
      ...
      setStatus("背景图生成成功，已自动选中。你可以继续切换其他场景。");
    } catch (error) {
  ...
  function updateDirectionalPos(axis, value) {
    const current = Array.isArray(sceneLight?.directionalPosition)
      ? [...sceneLight.directionalPosition]
      : [5, 10, 5];
  ...
  return (
    <ShellLayout
      title="场景预览"
      subtitle="选择或生成背景，并微调角色与灯光后进入交互展示。"
      backTo="/rig-preview"
    >
      <div className="two-column">
        <section className="glass-panel workflow-side-panel workflow-side-panel-scene">
          <h2>场景图库</h2>
          <p className="muted">
            输入关键词刷新图库，或上传 / 生成背景图，点击缩略图即可在右侧实时预览。
          </p>
              ...
              <div className="scene-editor-panel">
                ...
                <div className="scene-editor-scroll">
                  <section className="scene-editor-section" aria-label="角色位置">
                    ...
                    />
                    <label className="field-label">朝向角度：{Math.round(sceneAvatarRotationY)}°</label>
                    <input
                      type="range"
                      min="-180"
                      max="180"
                      step="1"
                      value={sceneAvatarRotationY}
                      onChange={(event) => setSceneAvatarRotationY(Number(event.target.value))}
                    />
                  </section>
                  ...
                  <section className="scene-editor-section" aria-label="镜头参数">
                    ...
                    <label className="field-label">
                      镜头距离：{(sceneCamera?.position?.[2] || 9.6).toFixed(2)}
                    </label>
                    <input
                      type="range"
                      min="6.8"
                  ...
                  <section className="scene-editor-section" aria-label="灯光参数">
                    ...
                    />
                    <label className="field-label">
                      主光强度：{(sceneLight?.directional || 1.35).toFixed(2)}
                    </label>
                    <input
                      type="range"
                      min="0.3"
                    ...
                    />
                    <label className="field-label">
                      主光 X：{(sceneLight?.directionalPosition?.[0] || 5).toFixed(2)}
                    </label>
                    <input
                      type="range"
                      min="-10"
                    ...
                    />
                    <label className="field-label">
                      主光 Y：{(sceneLight?.directionalPosition?.[1] || 10).toFixed(2)}
                    </label>
                    <input
                      type="range"
                      min="2"
        ...
        <section className="glass-panel preview-panel scene-preview-panel">
          ...
          <div className="animation-stage scene-animation-stage">
            ...
            >
              ...
              <directionalLight
                position={sceneLight?.directionalPosition || [5, 10, 5]}
                intensity={sceneLight?.directional || 1.35}
              />
              <Experience
                isWaving={false}
                setIsWaving={() => {}}
                ...
                previewAnimationUrl={
                  presetName
                    ? `/assets/presets/${presetName}/animations/Standing Idle.fbx`
